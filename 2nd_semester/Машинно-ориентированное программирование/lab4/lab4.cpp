...
extern "C" bool _cdecl compare(char* s1, char* s2, __int16 size);
extern "C" void _cdecl copy(char* s1, char* s2, __int16 size);
extern "C" void _cdecl insert(char* s1, char ch, __int16 pos);
extern "C" __int16 _cdecl search(char* s1, char ch);
...
int main() {
    ...
    char s2[] = "45";
    char ch = '2';
    __int16 size = strlen(s1);
    if (compare(s1, s2, size)) cout << "equal";
    else cout << "not equal";
    copy(s1, s2, size);
    insert(s1, ch, 0);
    cout << search(s1, ch) << '\n';
    ...
    cout << "Результирующая строка: " << str << endl;
